...
class Attendance:
    def __init__(self, root):
        ...
        title_lbl2 = Label(text="Attendance", font=("times new roman", 20, "bold"), fg="blue")
        ...
        # Choose Course frame
        choose_course_frame = LabelFrame(bd=2, relief=RIDGE, text="Choose Course", font=("times new roman", 12, "bold"))
        choose_course_frame.place(x=20, y=250, width=500, height=200)

        # Choose Course
        course_label = Label(choose_course_frame, text="Course", font=("times new roman", 12, "bold"))
        ...
        train_btn = Button(choose_course_frame, text="Train Data", font=("times new roman", 12, "bold"), width=15, command=self.train_classifier)
        train_btn.place(x=330, y=65)

        # Attendance Start frame
        attendance_frame = LabelFrame(bd=2, relief=RIDGE, text="Attendance Start", font=("times new roman", 12, "bold"))
        attendance_frame.place(x=20, y=500, width=500, height=200)

        # Attendance Start
        course_label = Label(attendance_frame, text="Attendance Start", font=("times new roman", 12, "bold"))
        ...
        start_btn = Button(attendance_frame, text="Start Attendance", font=("times new roman", 12, "bold"), width=15, command=self.face_recognition)
        start_btn.grid(row=0, column=2)

        # Right Frame
        Right_frame = LabelFrame(bd=2, relief=RIDGE, text="Attendance Chart", font=("times new roman", 12, "bold"))
    ...
    def train_classifier(self):
        if self.var_course.get() == "Select Course":
            ...
            file_path = [data_dir + "/" + file for file in os.listdir(data_dir)]
            faces = []
            ids = []
    ...
    def face_recognition(self):
        def draw_boundray(img, classifier, scaleFactor, minNeighbors, color, text, clf):
            ...
            for(x, y, w, h) in features:
                ...
                my_cursor.execute("SELECT Kor_Name FROM STUDENT WHERE Student_id = '" + str(id) + "'")
                ...
                if predict < 80:
                    ...
                    cv2.putText(img, f"{id} {name}", (x, y+h + 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255,255,255), 1)
                    self.mark_attendance(name, kor_name, id, self.var_attendance_course.get())
                else:
    ...
    def fetch_data(self):
        ...
        my_cursor = conn.cursor()
        my_cursor.execute("SELECT * FROM ATTENDANCE") #WHERE COURSE = '" + self.var_attendance_course.get() +"'")
